...
void Graphics::createPipeline(
    ...
    assert(
            ...
            "Cant create graphics pipeline: no pipelineLayout provided in configInfo"
            );
    assert(
            configInfo.renderPass != VK_NULL_HANDLE &&
            "Cant create graphics pipeline: no renderPass provided in configInfo"
            );
    ...
    assert(vkCreateGraphicsPipelines(device.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) == VK_SUCCESS);
}

void Graphics::destroyPipeline() {
    vkDestroyShaderModule(_device->device(), vertShader, nullptr);
    vkDestroyShaderModule(_device->device(), fragShader, nullptr);
